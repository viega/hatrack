...
static bool             lowhat1_store_put_if_empty(lowhat1_store_t *,
						   lowhat1_t *,
						   hatrack_hash_t *, void *);
...
static hatrack_view_t  *lowhat1_store_view(lowhat1_store_t *, lowhat1_t *,
					   uint64_t, uint64_t *);
...
const hatrack_vtable_t lowhat1_vtable = {
...
void *
lowhat1_get(lowhat1_t *self, hatrack_hash_t *hv, bool *found)
{
    void *ret;
...
void *
lowhat1_remove(lowhat1_t *self, hatrack_hash_t *hv, bool *found)
{
    void *ret;
                  ...
                  bool            *found)
{
    ...
    for (i = 0; i < self->last_slot; i++) {
        ...
        hv2.w1    = 0;
        hv2.w2    = 0;
        if (!LCAS(&ptrbucket->hv, &hv2, *hv1, LOWHAT1_CTR_BUCKET_ACQUIRE)) {
            if (!hatrack_hashes_eq(hv1, &hv2)) {
                bix = (bix + 1) & self->last_slot;
                continue;
            }
        ...
        hv2.w1 = 0;
        hv2.w2 = 0;
        LCAS(&bucket->hv, &hv2, *hv1, LOWHAT1_CTR_HIST_HASH);
...
found_history_bucket:
    ...
    if (!LCAS(&bucket->head, &head, candidate, LOWHAT1_CTR_REC_INSTALL)) {
                     ...
                     bool            *found)
{
    ...
    for (i = 0; i < self->last_slot; i++) {
        ...
        hv2.w1 = 0;
        hv2.w2 = 0;
        LCAS(&bucket->hv, &hv2, *hv1, LOWHAT1_CTR_BUCKET_ACQUIRE);
...
found_history_bucket:
    ...
    candidate->next = NULL;
    candidate->item = NULL;
    if (!LCAS(&bucket->head, &head, candidate, LOWHAT1_CTR_DEL)) {
        mmm_retire_unused(candidate);
...
lowhat1_do_migration(lowhat1_store_t *old, lowhat1_store_t *new)
{
    ...
    while (cur < store_end) {
        ...
        LCAS(&target->hv, &expected_hv, cur_hv, LOWHAT1_CTR_MIGRATE_HV);
        LCAS(&target->head, &expected_head, old_record, LOWHAT1_CTR_MIG_REC);
        ...
        //
        expected_ptr = NULL;
        LCAS(&ptr_bucket->ptr, &expected_ptr, target, LOWHAT1_CTR_NEW_PTR);
    ...
    LCAS(&new->hist_next, &expected_ptr, target, LOWHAT1_CTR_F_HIST);
...
#if !defined(HATRACK_DONT_SORT) && !defined(HATRACK_ALWAYS_USE_QSORT)
static inline void
lowhat1_insertion_sort(hatrack_view_t *view, uint64_t num_items)
{
    uint64_t       i, j;
    hatrack_view_t swap;
