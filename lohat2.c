...
static void            *lohat2_store_put          (lohat2_store_t *, lohat2_t *,
						   hatrack_hash_t *, void *,
						   bool *);
static bool             lohat2_store_put_if_empty (lohat2_store_t *,
						   lohat2_t *, hatrack_hash_t *,
						   void *);
static void            *lohat2_store_remove       (lohat2_store_t *, lohat2_t *,
						   hatrack_hash_t *, bool *);
static lohat2_store_t *lohat2_store_migrate       (lohat2_store_t *,
						   lohat2_t *);

#if !defined(HATRACK_DONT_SORT) && !defined(HATRACK_ALWAYS_USE_QSORT)
static void             lohat2_insertion_sort     (hatrack_view_t *, uint64_t);
#endif
...
lohat2_get(lohat2_t *self, hatrack_hash_t *hv, bool *found)
{
    ...
    mmm_start_basic_op();
    ret = lohat2_store_get(self->store_current, self, hv, found);
    mmm_end_op();
...
void *
lohat2_remove(lohat2_t *self, hatrack_hash_t *hv, bool *found)
{
    void *ret;

    mmm_start_basic_op();
    ret = lohat2_store_remove(self->store_current, self, hv, found);
    mmm_end_op();
...
void
lohat2_delete(lohat2_t *self)
{
    lohat2_store_t   *store   = atomic_load(&self->store_current);
    lohat2_history_t *buckets = store->hist_buckets;
    lohat2_history_t *p       = buckets;
    lohat2_history_t *end     = store->hist_end;
    lohat_record_t   *rec;
...
hatrack_view_t *
lohat2_view(lohat2_t *self, uint64_t *out_num)
{
    lohat2_history_t *cur;
    lohat2_history_t *end;
    ...
    end   = atomic_load(&store->hist_next);
    ...
    *out_num  = num_items;
    ...
    if (num_items >= HATRACK_QSORT_THRESHOLD) {
        ...
        lohat2_insertion_sort(view, num_items);
    }
...
lohat2_store_new(uint64_t size)
{
    ...
    sz    = sizeof(lohat2_store_t) + sizeof(lohat2_indirect_t) * size;
    store = (lohat2_store_t *)mmm_alloc_committed(sz);

    store->last_slot    = size - 1;
    store->threshold    = hatrack_compute_table_threshold(size);
    store->del_count    = ATOMIC_VAR_INIT(0);
    sz                  = sizeof(lohat2_history_t) * size;
    store->hist_buckets = (lohat2_history_t *)mmm_alloc_committed(sz);
...
static void
lohat2_retire_store(lohat2_store_t *self)
{
...
static void *
lohat2_store_get(lohat2_store_t *self,
                 lohat2_t       *top,
                 hatrack_hash_t *hv1,
                 bool           *found)
{
...
static void *
lohat2_store_put(lohat2_store_t *self,
                 lohat2_t       *top,
                 hatrack_hash_t *hv1,
                 void           *item,
                 bool           *found)
{
    ...
    for (i = 0; i < self->last_slot; i++) {
        ...
        hv2.w1    = 0;
        hv2.w2    = 0;
        if (!LCAS(&ptrbucket->hv, &hv2, *hv1, LOHAT2_CTR_BUCKET_ACQUIRE)) {
            if (!hatrack_hashes_eq(hv1, &hv2)) {
                bix = (bix + 1) & self->last_slot;
                continue;
            }
        ...
        if (!bucket) {
            ...
            if (new_bucket >= self->hist_end) {
                return lohat2_store_put(lohat2_store_migrate(self, top),
                                        top,
                                        hv1,
                                        item,
                                        found);
        ...
        hv2.w1 = 0;
        hv2.w2 = 0;
        LCAS(&bucket->hv, &hv2, *hv1, LOHAT2_CTR_HIST_HASH);
...
found_history_bucket:
    ...
    if (head && !hatrack_pflag_test(head->next, LOHAT_F_USED)) {
        ...
        if (LCAS(&bucket->fwd, &fwd, new_bucket, LOHAT2_CTR_FWD)) {
            mmm_retire(head);
        }
        // Could just goto above. For now, let's just make sure it
        // works.
        return lohat2_store_put(self, top, hv1, item, found);
    }
...
static bool
lohat2_store_put_if_empty(lohat2_store_t *self,
                          lohat2_t       *top,
                          hatrack_hash_t *hv1,
                          void           *item)
{
    ...
    for (i = 0; i < self->last_slot; i++) {
        ...
        if (!bucket) {
            new_bucket = atomic_fetch_add(&self->hist_next, 1);
            if (new_bucket >= self->hist_end) {
                return lohat2_store_put_if_empty(
                    lohat2_store_migrate(self, top),
                    top,
                    hv1,
                    item);
            }
    ...
    return lohat2_store_put_if_empty(lohat2_store_migrate(self, top),
                                     top,
                                     hv1,
                                     item);
...
static void *
lohat2_store_remove(lohat2_store_t *self,
                    lohat2_t       *top,
                    hatrack_hash_t *hv1,
                    bool           *found)
{
    ...
    for (i = 0; i < self->last_slot; i++) {
        ...
        hv2.w1 = 0;
        hv2.w2 = 0;
        LCAS(&bucket->hv, &hv2, *hv1, LOHAT2_CTR_BUCKET_ACQUIRE);
...
found_history_bucket:
    ...
    if (hatrack_pflag_test(head, LOHAT_F_MOVING)) {
        return lohat2_store_remove(lohat2_store_migrate(self, top),
                                   top,
                                   hv1,
                                   found);
    }
    ...
    candidate->next = NULL;
    candidate->item = NULL;
    if (!LCAS(&bucket->head, &head, candidate, LOHAT2_CTR_DEL)) {
        mmm_retire_unused(candidate);
...
static lohat2_store_t *
lohat2_store_migrate(lohat2_store_t *self, lohat2_t *top)
{
    lohat2_store_t    *new_store;
    lohat2_store_t    *candidate;
    uint64_t           new_size;
    lohat2_history_t  *cur;
    lohat2_history_t  *target;
    lohat2_history_t  *store_end;
    lohat_record_t    *old_head;
    lohat_record_t    *deflagged;  // Head w/ migration flags removed.
    lohat2_indirect_t *ptr_bucket; // New unordered bucket location.
    ...
    while (cur < store_end) {
        ...
        LCAS(&target->hv, &expected_hv, cur_hv, LOHAT2_CTR_MIGRATE_HV);
        LCAS(&target->head, &expected_head, deflagged, LOHAT2_CTR_MIG_REC);
    ...
    LCAS(&new_store->hist_next, &expected_ptr, target, LOHAT2_CTR_F_HIST);

    if (LCAS(&top->store_current, &self, new_store, LOHAT2_CTR_STORE_INSTALL)) {
        lohat2_retire_store(self);
    }
...
#if !defined(HATRACK_DONT_SORT) && !defined(HATRACK_ALWAYS_USE_QSORT)
static inline void
lohat2_insertion_sort(hatrack_view_t *view, uint64_t num_items)
{
    uint64_t       i, j;
    hatrack_view_t swap;
