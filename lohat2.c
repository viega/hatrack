...
lohat2_get(lohat2_t *self, hatrack_hash_t *hv, bool *found)
{
    ...
    ret   = lohat2_store_get(store, self, hv, found);
    mmm_end_op();
...
bool
lohat2_add(lohat2_t *self, hatrack_hash_t *hv, void *item)
{
    bool            ret;
    lohat2_store_t *store;
    ...
    mmm_start_basic_op();
    store = atomic_read(&self->store_current);
    ret   = lohat2_store_add(store, self, hv, item);
    mmm_end_op();
...
lohat2_remove(lohat2_t *self, hatrack_hash_t *hv, bool *found)
{
    ...
    mmm_start_basic_op();
    store = atomic_read(&self->store_current);
    ret   = lohat2_store_remove(store, self, hv, found);
    mmm_end_op();
...
static bool
lohat2_store_add(lohat2_store_t *self,
                          lohat2_t       *top,
                          hatrack_hash_t *hv1,
                          void           *item)
{
...
migrate_and_retry:
    self = lohat2_store_migrate(self, top);
    return lohat2_store_add(self, top, hv1, item);
