...
static void refhat_a_migrate(refhat_a_t *);
...
refhat_a_init(refhat_a_t *self)
{
    ...
    self->used_count = 0;
    self->item_count = 0;
    self->next_epoch = 1; // 0 is reserved to mean 'deleted' or 'not present'
    self->buckets
        = (refhat_a_bucket_t *)calloc(size, sizeof(refhat_a_bucket_t));
}
 ...
 */
void *
refhat_a_get(refhat_a_t *self, hatrack_hash_t hv, bool *found)
{
    uint64_t           bix;
    uint64_t           i;
 ...
 */
void *
refhat_a_put(refhat_a_t *self, hatrack_hash_t hv, void *item, bool *found)
{
    uint64_t           bix;
    uint64_t           i;
    refhat_a_bucket_t *cur;
    ...
    for (i = 0; i <= self->last_slot; i++) {
        ...
        if (hatrack_bucket_unreserved(cur->hv)) {
            if (self->used_count + 1 == self->threshold) {
                refhat_a_migrate(self);
                return refhat_a_put(self, hv, item, found);
            }
            self->used_count++;
            self->item_count++;
 ...
 */
void *
refhat_a_replace(refhat_a_t *self, hatrack_hash_t hv, void *item, bool *found)
{
    uint64_t           bix;
    uint64_t           i;
    refhat_a_bucket_t *cur;
 ...
 */
bool
refhat_a_add(refhat_a_t *self, hatrack_hash_t hv, void *item)
{
    uint64_t           bix;
    uint64_t           i;
    refhat_a_bucket_t *cur;
    ...
    for (i = 0; i <= self->last_slot; i++) {
        ...
        if (hatrack_bucket_unreserved(cur->hv)) {
            if (self->used_count + 1 == self->threshold) {
                refhat_a_migrate(self);
                return refhat_a_add(self, hv, item);
            }
            self->used_count++;
            self->item_count++;
            cur->hv = hv;
 ...
 */
void *
refhat_a_remove(refhat_a_t *self, hatrack_hash_t hv, bool *found)
{
    uint64_t           bix;
    uint64_t           i;
    refhat_a_bucket_t *cur;
 ...
 */
void
refhat_a_delete(refhat_a_t *self)
{
    free(self->buckets);
    free(self);
}
 ...
 */
hatrack_view_t *
refhat_a_view(refhat_a_t *self, uint64_t *num, bool sort)
{
    hatrack_view_t    *view;
    hatrack_view_t    *p;
    refhat_a_record_t  record;
    refhat_a_bucket_t *cur;
    refhat_a_bucket_t *end;
 ...
 */
static void
refhat_a_migrate(refhat_a_t *self)
{
    refhat_a_bucket_t *new_buckets;
    refhat_a_bucket_t *cur;
    refhat_a_bucket_t *target;
    refhat_a_record_t  record;
    ...
    bucket_size   = sizeof(refhat_a_bucket_t);
    num_buckets   = hatrack_new_size(self->last_slot, self->item_count + 1);
    new_last_slot = num_buckets - 1;
    new_buckets   = (refhat_a_bucket_t *)calloc(num_buckets, bucket_size);

    for (n = 0; n <= self->last_slot; n++) {
        cur = &self->buckets[n];

        // Bucket hasn't been used.
        if (hatrack_bucket_unreserved(cur->hv)) {
            continue;
        }
        ...
        // Item in the bucket was deleted.
        if (!record.epoch) {
            continue;
        }
